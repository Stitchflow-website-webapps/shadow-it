...
async function sendNewAppsWebhookNotification(organizationId: string, newAppNames: string[], orgDomain: string) {
  if (!newAppNames || newAppNames.length === 0) {
    console.log(`[TestCron:Microsoft:${orgDomain}] No new apps to send via webhook`);
    return;
  }
  ...
  try {
    ...
    console.log(`[TestCron:Microsoft:${orgDomain}] Sending webhook notification for ${newAppNames.length} newly discovered apps`);
    ...
    if (response.ok) {
      ...
      console.log(`[TestCron:Microsoft:${orgDomain}] Webhook notification sent successfully:`, responseData);
    } else {
      const errorData = await response.text();
      console.error(`[TestCron:Microsoft:${orgDomain}] Failed to send webhook notification. Status: ${response.status}, Response: ${errorData}`);
    }
  } catch (error) {
    console.error(`[TestCron:Microsoft:${orgDomain}] Error sending webhook notification:`, error);
  }
}