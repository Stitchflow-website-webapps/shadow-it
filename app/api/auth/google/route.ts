  ...
  try {
    ...
    if ((!existingUser || !hasAdminAccess) && (!hasRequiredAdminScopes || !hasCompletedConsent) && (!oauthTokens.refresh_token || !hasRequiredAdminScopes)) {
      ...
      const redirectUri = process.env.NODE_ENV === 'production' 
        ? 'https://www.manage.stitchflow.io/api/auth/google'
    ...
    if (!userInfo.hd) {
      ...
      return NextResponse.redirect(new URL('https://www.manage.stitchflow.io/?error=not_workspace_account', request.url));
    }
    ...
    try {
      ...
      if (existingOrganizeOrgByDomain) {
        ...
        console.log('No existing organize-app-inbox organization found for domain:', userInfo.hd, '- creating new one');
        ...
        if (organizeOrgError) {
          ...
          console.log('Created new organize-app-inbox organization for:', userInfo.hd);
        }
      }
    } catch (error) {
